...
system::system(const std::shared_ptr<stella_vslam::system>& slam,
    ...
    while (true) {
        try {
            auto gmt = tf_->lookupTransform(camera_frame_, robot_base_frame_, tf2::TimePointZero);
            tf2::fromMsg(gmt.transform,camera_to_base_link_ );
            break;
        } catch (tf2::TransformException& ex) {
            RCLCPP_DEBUG(node_->get_logger(), "Transform failed: %s", ex.what());
        }
    }
    RCLCPP_INFO(node_->get_logger(), "System initialized : Setting Parameters Done X = %f, Y = %f", 
                                     camera_to_base_link_.getOrigin().getX(), 
                                     camera_to_base_link_.getOrigin().getY());
}
...
void system::publish_pose(const std::shared_ptr<Eigen::Matrix4d> cam_pose_wc, const rclcpp::Time& stamp) {
    ...
    if (camera_backwards_) {
        // Define a 180 degrees rotation around Z-axis
        Eigen::AngleAxisd rotation180(M_PI, Eigen::Vector3d::UnitZ());
        // Apply the 180-degree rotation to the original transform
        map_to_camera = rotation180 * map_to_camera;
...
void system::publish_keyframes(const rclcpp::Time& stamp) {
    ...
    for (const auto& keyfrm : all_keyfrms) {
        ...
        if (camera_backwards_) {
            ...
            Eigen::AngleAxisd rotation180(M_PI, Eigen::Vector3d::UnitZ());
            // Apply the 180-degree rotation to the original transform
            pose_affine = rotation180 * pose_affine;
            // Translate back on X & Z to camera frame
            Eigen::Translation3d pb (camera_to_base_link_.getOrigin().x(), 0, camera_to_base_link_.getOrigin().z());
            pose_affine = pb * pose_affine;
            keyframes_msg.poses.push_back(tf2::toMsg(pose_affine*tf2TransformToEigenAffine3d(camera_to_base_link_.inverse())));
        } else {
...
void system::setParams() {
    ...
    RCLCPP_INFO(node_->get_logger(), "map_frame: %s", map_frame_.c_str());

    robot_base_frame_ = std::string("base_link");
    robot_base_frame_ = node_->declare_parameter("robot_base_frame", robot_base_frame_);
    RCLCPP_INFO(node_->get_logger(), "robot_base_frame: %s", robot_base_frame_.c_str());

    camera_frame_ = std::string("camera_frame");
    camera_frame_ = node_->declare_parameter("camera_frame", camera_frame_);
    RCLCPP_INFO(node_->get_logger(), "camera_frame: %s", camera_frame_.c_str());

    publish_tf_ = true;
    publish_tf_ = node_->declare_parameter("publish_tf", publish_tf_);
    RCLCPP_INFO(node_->get_logger(), "publish_tf: %d", publish_tf_);

    odom2d_ = false;
    odom2d_ = node_->declare_parameter("odom2d", odom2d_);
    RCLCPP_INFO(node_->get_logger(), "odom2d: %d", odom2d_);

    publish_keyframes_ = true;
    publish_keyframes_ = node_->declare_parameter("publish_keyframes", publish_keyframes_);
    RCLCPP_INFO(node_->get_logger(), "publish_keyframes: %d", publish_keyframes_);

    transform_tolerance_ = 0.5;
    transform_tolerance_ = node_->declare_parameter("transform_tolerance", transform_tolerance_);
    RCLCPP_INFO(node_->get_logger(), "transform_tolerance: %f", transform_tolerance_);

    encoding_ = "";
    encoding_ = node_->declare_parameter("encoding", encoding_);
    if (!encoding_.empty()) {
        RCLCPP_INFO(node_->get_logger(), "Encoding: %s", encoding_.c_str());
    } else {
        RCLCPP_INFO(node_->get_logger(), "Encoding: ''");
    }

    img_capture_ = false;
    img_capture_ = node_->declare_parameter("img_capture", img_capture_);
    RCLCPP_INFO(node_->get_logger(), "img_capture: %d", img_capture_);

    camera_backwards_ = false;
    camera_backwards_ = node_->declare_parameter("camera_backwards", camera_backwards_);
    RCLCPP_INFO(node_->get_logger(), "camera_backwards: %d", camera_backwards_);

    img_overlay_stats_ = false;
    img_overlay_stats_ = node_->declare_parameter("img_overlay_stats", img_overlay_stats_);
    RCLCPP_INFO(node_->get_logger(), "img_overlay_stats: %d", img_overlay_stats_);
    ...
    if (!img_capture_path_.empty()) {
        RCLCPP_ERROR(node_->get_logger(), "img_capture_path: %s", img_capture_path_.c_str());
    }
    ...
    RCLCPP_INFO(node_->get_logger(), "img_capture_distance_thr: %f", img_capture_distance_thr_);

    img_capture_angle_thr_ = 0.5;
    img_capture_angle_thr_ = node_->declare_parameter("img_capture_angle_thr", img_capture_angle_thr_);
    RCLCPP_INFO(node_->get_logger(), "img_capture_angle_thr: %f", img_capture_angle_thr_);
} 
...
void system::capture_image(const cv::Mat& img) {
    ...
    if (distance >= img_capture_distance_thr_ || angle >= img_capture_angle_thr_) {
        ...
        RCLCPP_INFO(node_->get_logger(), "Capture image %f %f %d -> %s", distance, angle , img_count, img_path.c_str());
...
void stereo::callback(const sensor_msgs::msg::Image::ConstSharedPtr& left, const sensor_msgs::msg::Image::ConstSharedPtr& right) {
    RCLCPP_DEBUG_STREAM_THROTTLE(node_->get_logger(), *node_->get_clock(), 1000, "Received stereo images");
    if (camera_optical_frame_.empty()) {
    ...
    try {
        ...
        RCLCPP_ERROR(node_->get_logger(), "Exception in feed_stereo_frame: %s", e.what());
    }
    if (publish_keyframes_) {
